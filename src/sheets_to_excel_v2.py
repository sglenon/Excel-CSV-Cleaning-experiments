"""
Excel Sheet Splitter (v2)

Splits each sheet of an Excel (.xlsx) file into a separate Excel file using openpyxl.
- Sheet titles are truncated to 63 characters, PostgreSQL-friendly, and use abbreviations heuristically.
- Preserves cell formatting, formulas, merged cells, and styles, but may not preserve charts or images.
- Works on Windows, macOS, and Linux.

Usage:
    python sheets_to_excel_v2.py <input_excel_file.xlsx> <output_folder>
...
Example:
    python sheets_to_excel_v2.py "MyWorkbook.xlsx" "sheets_to_excel/"
...
def abbreviate_sheet_name(sheet_name):
    # Heuristic: Use first word or uppercase letters as abbreviation, then rest of name
    words = re.split(r'[\s_-]+', sheet_name)
    if len(words) > 1:
        abbr = words[0]
    else:
        # fallback: use uppercase letters as abbreviation
        abbr = ''.join([c for c in sheet_name if c.isupper()])
        if not abbr:
            abbr = sheet_name[:4]
    # Remove nonsense characters and make PostgreSQL-friendly
    cleaned = re.sub(r'[^A-Za-z0-9_]', '', abbr)
    rest = re.sub(r'[^A-Za-z0-9_]', '', '_'.join(words[1:]))
    title = (cleaned + ('_' + rest if rest else '')).lower()
...
def separate_sheets_with_openpyxl(input_file, output_folder):
    ...
    for idx, sheet_name in enumerate(source_wb.sheetnames, start=1):
        ...
        # Create a new sheet in the new workbook with the abbreviated, cleaned title
        abbr_title = abbreviate_sheet_name(sheet_name)
        new_sheet = new_wb.create_sheet(title=abbr_title)
